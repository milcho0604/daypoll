...
| 메인 | 방 생성 | 링크 발급 (QR + 카톡 문구) |
|---|---|---|
| ![main](docs/screenshots/main.png) | ![create](docs/screenshots/create.png) | ![share](docs/screenshots/share.png) |
...
브라우저 (모바일/데스크탑)
   ↓ HTTPS
Vercel (Next.js)   ←— git push 자동 배포
   ↓ HTTPS API 호출
https://<host>.<tailnet>.ts.net  ←— Tailscale Funnel (영구 URL, 도메인 없이)
   ↓
[ 맥미니 / Docker ]
  whenever-api (NestJS, 127.0.0.1:3001)
  whenever-postgres (호스트 비노출)
```
