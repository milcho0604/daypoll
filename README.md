...
| 링크 발급 (QR + 카톡 문구) | 방 화면 (zinc 베이스 + 🏆 amber 1위) |
...
4. /rooms/[id]       실시간 순위(🏆 amber), D-day, 마감 시 1위 칩 탭 → 누가 가능한지 팝업, .ics 다운로드
```
...
| **자동 복원** | 같은 브라우저 재방문 → PIN 없이도 자동. 다른 기기는 **PIN 만 입력** (닉네임 없이) → 같은 PIN 충돌 시에만 닉네임 fallback. **한 사람 = 한 토큰** (multi-device — 폰/PC 동시 살아있음) |
| **결과** | 실시간 순위표 (날짜별 ⇄ 사람별 토글), 1위 🏆 amber 그라데이션, 동률 1위 공동 표시 (6개까지 + 더보기), Socket.IO + 폴링 fallback |
| **마감** | D-day 표시, D-3 이내 rose 옅게 / D-1 이내 rose 진하게, 마감 후 잠금. 개설자는 언제든 수정/재개 |
| **결과 활용** | 마감 시 확정 1위 칩 탭 → **누가 가능한지 팝업** (bottom-sheet/모달). 1위 날짜 `.ics` 다운로드, **결과 이미지 저장** (OG 카드 PNG — 단톡방 캡쳐 공유), OG 카드 자동 생성 |
...
| **공유 카드** | 메인·방 모두 OG 이미지 자동 생성 (1200×630 zinc 그라데이션 + amber 1위 강조). 카톡/슬랙/트위터 미리보기 |
...
| **디자인 톤** | 베이스 zinc 무채색 + amber 는 트로피·1등·확정 hot path 한정 (4곳). 시멘틱 rose = 마감 임박. CLAUDE.md §1-1 토큰 가이드 |
| **a11y** | focus ring (zinc-900/40 강조), aria-live (저장/에러 메시지), **`prefers-reduced-motion` 존중** (애니메이션 자동 끔) |
...
<p align="center">
  <img src="docs/architecture/architecture.svg.png" alt="moilga 서버 아키텍처" width="100%">
</p>

> 라이브 요청은 **amber 경로**(브라우저 → Cloudflare → 터널 → API), 점선은 스케줄·배포. 외부 인바운드 포트는 **0개** — `cloudflared` 가 바깥으로만 연결한다. (위는 3200px PNG · 원본 벡터: [architecture.svg](docs/architecture/architecture.svg))

<details>
<summary>흐름 요약 · mermaid 소스 (편집용)</summary>

- **페이지** — 브라우저 → Vercel(`moilga.com`, Next.js 15): SSR/정적, 글로벌 CDN
- **API + 실시간** — 브라우저 → `api.moilga.com`(Cloudflare Edge: TLS·WAF·DDoS·cf-connecting-ip) → outbound QUIC 터널 → `cloudflared`(맥미니 호스트 데몬) → `localhost:3001`
- **백엔드** — `moilga-api`(NestJS 11 · REST + WebSocket socket.io · Sentry · rate-limit · cap_drop ALL · USER node) ↔ `moilga-postgres`(16-alpine · 호스트 비노출 · `<1ms`)
- **운영** — `moilga-backup`(매일 04:30 → 외장 SSD · 14일/4주 보존) · `moilga-cleanup`(매일 `rooms > 90d` DELETE CASCADE)
- **배포** — `git push main` → GitHub Actions(CI: lint·타입체크·빌드·e2e) → Deploy(ubuntu → Tailscale ephemeral ssh → 맥미니 `docker-compose up -d --build` → `/health` 검증). 프론트는 Vercel git 연동 자동 배포
- **모니터링** — `cert-check`(TLS 만료 · 매일 11:00 KST) · `uptime`(`/health` · 매 10분)

```mermaid
flowchart LR
  classDef front fill:#fafafa,stroke:#71717a,color:#18181b
  classDef edge fill:#f4f4f5,stroke:#52525b,color:#18181b
  classDef back fill:#fffbeb,stroke:#f59e0b,color:#5c3d1a
  classDef data fill:#fafafa,stroke:#71717a,color:#18181b
  classDef ops fill:#fafafa,stroke:#a1a1aa,color:#3f3f46
  classDef ci fill:#fafafa,stroke:#d4d4d8,color:#3f3f46
  ...
  API[moilga-api<br/>NestJS · 127.0.0.1:3001]:::back
  PG[(PostgreSQL<br/>호스트 비노출)]:::data
  Backup[moilga-backup<br/>postgres-backup-local<br/>매일 04:30]:::ops
  Cleanup[moilga-cleanup<br/>매일 1회 DELETE 90d+]:::ops
  ...
  API -->|pg| PG
  Backup -.->|pg_dump → /Volumes/milcho_ex/moilga-backups| PG
...
| Docker `moilga-backup` | 매일 04:30 (컨테이너 cron) | `pg_dump` → `/Volumes/milcho_ex/moilga-backups/{daily,weekly,monthly,last}/`. 14일/4주 rotation. `prodrigestivill/postgres-backup-local:16` |
| Docker `moilga-cleanup` | 매일 1회 (sleep 86400) | 90일 지난 `rooms` CASCADE DELETE. `postgres:16-alpine` + `psql` |
