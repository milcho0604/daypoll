...
![언제모여 서버 아키텍처](docs/architecture/architecture.svg)

> 라이브 요청은 **amber 경로**(브라우저 → Cloudflare → 터널 → API), 점선은 스케줄·배포. 외부 인바운드 포트는 **0개** — `cloudflared` 가 바깥으로만 연결한다. (벡터 [SVG](docs/architecture/architecture.svg) · 래스터 [PNG](docs/architecture/architecture.svg.png))
...
- **백엔드** — `whenever-api`(NestJS 11 · REST + WebSocket socket.io · Sentry · rate-limit · cap_drop ALL · USER node) ↔ `whenever-postgres`(16-alpine · 호스트 비노출 · `<1ms`)
- **운영** — `whenever-backup`(매일 04:30 → 외장 SSD · 14일/4주 보존) · `whenever-cleanup`(매일 `rooms > 90d` DELETE CASCADE)
