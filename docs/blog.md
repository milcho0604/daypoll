...
`visibility: private` 글은 공개 목록·RSS·사이트맵·검색엔진·공개 OG 이미지에서 제목과 본문을 숨긴다. 글 URL에서는 관리자 권한과 분리된 `BLOG_PRIVATE_TOKEN`으로 인증한 뒤에만 본문을 내려받는다. 원본 토큰은 브라우저 저장소에 남기지 않고 서버가 발급한 30분짜리 서명 `HttpOnly` 쿠키만 사용한다. 연속 실패는 10분 동안 제한되며 글 화면의 **다시 잠그기**로 즉시 세션을 지울 수 있다.

`BLOG_PRIVATE_TOKEN`은 16자 이상의 별도 고강도 값으로 Vercel Production·Preview 환경에 설정한다. `ADMIN_TOKEN`과 같은 값을 사용하지 않는다. 인증 감사 로그에는 성공·실패·잠금 이벤트와 해시된 글 식별자만 남고 토큰·IP·제목·본문은 남지 않는다.
